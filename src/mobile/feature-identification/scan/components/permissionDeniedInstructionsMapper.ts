...
const TRANSLATIONSTEPS_PATH = "identification.mobile.scan.permissionDenied.steps"

const getStepTranslation = (variant: string, stepIndex: number, subStepIndex?: number) =>
    `${TRANSLATIONSTEPS_PATH}.${variant}.step${stepIndex}${subStepIndex !== undefined ? `.${subStepIndex}` : ''}`;
