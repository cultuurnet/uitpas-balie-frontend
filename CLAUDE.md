...
Base components live in `src/ui/shadcn/` and are generated by the shadcn CLI. **Never edit these files directly** — they get overwritten when updated via `yarn dlx shadcn add <component>`.
...
- Default to Server Components; add `'use client'` only when you need state, event handlers, effects, or browser APIs
- `'use client'` is a boundary — the entire import tree below it runs client-side; mark only the smallest components that need it, not their parents
- Route `params` is a `Promise` in Next.js 16 — always `await params` in `page.tsx` and `layout.tsx`
- Use `use(promise)` in Client Components to consume promises passed from Server Components (wrap in `<Suspense>`)
